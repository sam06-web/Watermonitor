  ...
  async searchRivers(query) {
    ...
    const parseItems = (items) => {
      ...
      for (const item of items) {
        ...
        const itemCat = item.category || item.class || '';
        const isWaterFeature = itemCat === 'waterway'
          || (itemCat === 'natural' && ['water', 'lake', 'reservoir', 'river', 'riverbank'].includes(itemType))
          || ['river', 'lake', 'reservoir', 'stream', 'canal', 'riverbank', 'collection'].includes(itemType)
          || /\b(river|lake|reservoir|canal|stream|flood carrier|aaru|aru)\b/i.test(item.display_name || '');
