...
import { getRiverAliasIds } from '../db/presetRivers.js';
...
function cachedImagePath(riverQuery, dateStr, type) {
  const primary = path.join(IMAGE_CACHE_DIR, `${safeFileName(riverQuery)}-${dateStr}-${type}.png`);
  if (fs.existsSync(primary)) return primary;
  const aliases = getRiverAliasIds(riverQuery);
  for (const alias of aliases) {
    const candidate = path.join(IMAGE_CACHE_DIR, `${safeFileName(alias)}-${dateStr}-${type}.png`);
