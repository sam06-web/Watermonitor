...
export default function RiverMap({ riverData, onAreaScanned }) {
  ...
  const flyToRiver = (map, riverData) => {
    ...
    if (hasValidBounds) {
      map.fitBounds(
        ...
        { padding: { top: 80, bottom: 80, left: 80, right: 80 }, maxZoom: 13, duration: 1800, pitch: 35 }
      );
  ...
  useEffect(() => {
    ...
    const setupRiverLayer = () => {
      ...
      const riverGeojson = {
        ...
        properties: { name: riverData.name, basin: riverData.basin }
      };
      ...
      if (geojsonSource) {
        ...
        map.addSource('river-vector', { type: 'geojson', data: riverGeojson });
        map.addLayer({
          id: 'river-glow',
          type: 'line',
          source: 'river-vector',
      ...
      // Always fly to the new river — two rAF ticks ensure the source data
      // has been committed to the GL context before the camera moves.
      requestAnimationFrame(() => requestAnimationFrame(() => flyToRiver(map, riverData)));
    };

    if (map.isStyleLoaded()) {
      setupRiverLayer();
    } else {
      map.once('load', setupRiverLayer);
    }
  }, [riverData]);
