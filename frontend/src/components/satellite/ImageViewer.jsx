...
export default function ImageViewer({ riverData, observation, onShowToast }) {
  ...
  const getImageUrl = (type, date, variant = 'latest') => {
    ...
    return getApiUrl(`/api/satellite/image?river=${encodeURIComponent(riverId)}&type=${encodeURIComponent(type)}&date=${encodeURIComponent(imageDate)}&scene=${encodeURIComponent(`${scene}-${variant}`)}`);
  };
