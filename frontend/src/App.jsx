...
function App() {
  ...
  const [leakThreshold] = useState(0.5);
  ...
  const [selectedSatelliteWaterBody, setSelectedSatelliteWaterBody] = useState(null);
  ...
  const [waterQualityMetrics, setWaterQualityMetrics] = useState({ ph: 0, turbidity: 0, temperature: 0 });
  ...
  useEffect(() => {
    if (!selectedSatelliteWaterBody) return;
    const controller = new AbortController();
    fetch(getApiUrl(`/api/satellite/latest?river=${encodeURIComponent(selectedSatelliteWaterBody)}`), { signal: controller.signal })
  ...
  // Ask the ML model for a prediction whenever sensor readings change.
  useEffect(() => {
    const ph = Number(waterQualityMetrics.ph);
    const tds = Number(realTimeMetrics.tds);
    const turbidity = Number(waterQualityMetrics.turbidity);

    // Only predict when we have valid sensor measurements (pH between 0.1 and 14, TDS >= 0, Turbidity >= 0)
    if (!Number.isFinite(ph) || !Number.isFinite(tds) || !Number.isFinite(turbidity) || ph <= 0 || ph > 14 || tds < 0 || turbidity < 0) {
      return;
    }

    const sample = { ph, tds, turbidity };
    const controller = new AbortController();
    dispatchModelStatus('loading');
    fetch(getApiUrl('/api/water-quality/predict'), {
      method: 'POST',
    ...
    try {
      ...
      client.on('message', (receivedTopic, message) => {
        ...
        try {
          ...
          const tempVal = payload.temperature ?? payload.temp ?? payload.temp_c ?? payload.temperature_c;
          if (tempVal !== undefined && Number.isFinite(Number(tempVal))) {
            setWaterQualityMetrics(prev => ({ ...prev, temperature: Number(tempVal) }));
          }

          const tdsVal = payload.tds_ppm ?? payload.tds ?? payload.TDS ?? payload.ppm;
          if (tdsVal !== undefined && Number.isFinite(Number(tdsVal))) {
            setRealTimeMetrics(prev => ({ ...prev, tds: Number(tdsVal) }));
          }
